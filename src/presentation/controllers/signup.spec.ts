...
const makeAddAccount = (): AddAccount => {
  class AddAccountStub implements AddAccount {
    async add (account: Account): Promise<AccountModel> {
      const fakeAccount = {
        id: 'valid_id',
        name: 'valid_name',
...
describe('SignUp Controller', () => {
  ...
  test('Shoud return 400 if no confirmPassword is provided', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Should call AddAccount with correct values', async () => {
    ...
    const accountSpy = jest.spyOn(addAccountStub, 'add')
    const httpRequest = {
      body: {
        name: 'valid_name',
        email: 'valid_email@mail.com',
